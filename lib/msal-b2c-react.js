...
function loginAndAcquireToken(successCallback) {
  ...
  if (!user) {
    ...
    if (state.noScopes) {
      ...
      localMsalApp.acquireTokenSilent(appConfig.scopes, null, null, "&login_hint&domain_hint=organizations" + appConfig.extraQueryParameters).then(function (accessToken) {
        ...
        if (error) {
          if (appConfig.silentLoginOnly) state.errorApp();else localMsalApp.loginRedirect(appConfig.scopes, appConfig.extraQueryParameters);
        }
      });
    }
  } else {
    ...
    if (state.noScopes) {
      ...
      // get access token
      localMsalApp.acquireTokenSilent(appConfig.scopes, null, null, appConfig.extraQueryParameters).then(function (accessToken) {
        state.accessToken = accessToken;
        if (state.launchApp) {
          state.launchApp();
        }
